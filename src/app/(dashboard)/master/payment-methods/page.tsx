...
export default function PaymentMethodsPage() {
  const { data, isLoading, error, mutate } = useSWR("/api/master/payment-method", getFetcher, { revalidateOnFocus: false });
  const items: PaymentMethod[] = Array.isArray(data) ? data : (data?.data ?? []);
  ...
  const handleSave = async () => {
    setLoading(true);
    if (editing) {
      await putFetcher(`/api/admin/master/payment-methods/${editing.id}`, { name, iconUrl });
    } else {
      await postFetcher("/api/admin/master/payment-methods", { name, iconUrl });
    }
    await mutate();
    setDialogOpen(false);
    setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    ...
    setLoading(true);
    await deleteFetcher(`/api/admin/master/payment-methods/${id}`, {});
    await mutate();
    setLoading(false);
  };
        ...
        <Table>
          ...
          <TableBody>
            {isLoading
              ...
              ? (
                <TableRow>
                  <TableCell colSpan={3}><Typography color="text.secondary">No payment methods found</Typography></TableCell>
                </TableRow>
              )
              : items.map((item) => (
        ...
        <DialogContent>
          <Stack gap={2} mt={1}>
            ...
            <TextField
              ...
              placeholder="https://cdn.simpleicons.org/paypal"
