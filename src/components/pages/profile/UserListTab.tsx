...
const UserListTab = ({ username, type }: { username: string; type: "followers" | "following" }) => {
  ...
  const getKey = (pageIndex: number, previousPageData: any) => {
    ...
    return [`/api/user/${username}/${type}`, params];
  };
  ...
  const pages = data ?? [];
  const users: FollowUser[] = pages.flatMap((p) => p?.data ?? []);
      ...
      <Grid container spacing={2}>
        {users.map((u) => (
          <Grid key={u.username} size={{ xs: 12, sm: 6, md: 4 }}>
            ...
            >
              <Avatar src={u.profilePictureUrl ?? undefined} sx={{ width: 44, height: 44 }}>
                {u.displayName[0]}
              </Avatar>
              <Box>
